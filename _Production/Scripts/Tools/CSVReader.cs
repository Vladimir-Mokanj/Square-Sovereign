...
public class CSVReader : IDisposable
{
    ...
    public CSVReader(string csvData) : this(Encoding.UTF8.GetBytes(csvData))
    {
    }

    public CSVReader(byte[] csvData)
    {
        if (csvData == null)
            throw new ArgumentNullException("Null byte[] passed to CSVReader");
        this.reader = new BinaryReader(new MemoryStream(csvData));
    }
    ...
    private object ReadNextObject()
    {
        ...
        {
            //Return everything as string because StringConverter.cs has problems with locale when parsing decimal values
            //
    ...
    public static DataTable ReadCSVData(string data, bool headerRow)
    {
        using (CSVReader reader = new CSVReader(data))
            return reader.CreateDataTable(headerRow);
    }
