...
{
	public Mesh GenerateAndConnectWireframeMesh(byte rows, byte cols, Vector3[] vertices)
	{
		SurfaceTool st = new();
		...
		for (byte x = 0; x < rows; x++)
			for (byte z = 0; z < cols; z++)
			{
				...
				if (vertices[br].Y > vertices[tl].Y && vertices[br].Y > vertices[tr].Y && vertices[br].Y > vertices[bl].Y ||
				    ...
				    vertices[tl].Y < vertices[tr].Y && vertices[tl].Y < vertices[bl].Y && vertices[tl].Y < vertices[br].Y)
				{
					st.AddVertex(vertices[tl]);
